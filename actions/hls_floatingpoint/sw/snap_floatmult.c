...
#include <libsnap.h>
#include <action_floatingpoint.h>
...
static void snap_prepare_floatmult(struct snap_job *cjob,
				 struct floatmult_job *mjob,
				 void *addr_in,
				 uint32_t size_in,
				 uint8_t type_in,
				 void *addr_out,
				 ...
				 uint8_t type_out)
{
	fprintf(stderr, "  prepare floatmult job of %ld bytes size\n", sizeof(*mjob));
...
int main(int argc, char *argv[])
{
	...
	action = snap_attach_action(card, FLOATMULT_ACTION_TYPE, action_irq, 60);
	if (action == NULL) {
	...
	snap_prepare_floatmult(&cjob, &mjob,
	...
	fprintf(stdout, "SNAP floatmult took %lld usec\n",
